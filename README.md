...
1. **Before the auction:** log in as admin, go to *Players*, and either add all 64 players one by one or use *Bulk CSV Upload* (see `sample_players.csv` for the expected columns: `name, role, base_price, stats, stars, photo_path`). `base_price` is in crore and is **₹30 Cr** for every player. `stars` is 2.0 to 5.0 in 0.5 steps. Set `stats` to the player affiliation: **Zoho Chennai**, **Zoho Kottarakara**, **Non-Zoho**, **Ex Zoho**, or **Ex ZoLiga** (shown as a badge on player cards). For bulk import, put player photos in `import/photos/` on the server and reference them in the `photo_path` column (filename only, e.g. `arjun.jpg`, or a path relative to that folder). Absolute paths work when importing on the same machine (e.g. local dev). Go to *Teams* and confirm the 8 teams: **₹1,000 Cr purse** and **8 slots** each (created via `seed.py`).
